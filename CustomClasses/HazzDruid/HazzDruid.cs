...
namespace HazzDruid
{
    class class2 : CombatRoutine
    {
            ...
            if (tar != null)
            {
                ...
                else
                {
					...
					if (hp < HazzDruidSettings.Instance.NaturesPercent && !isAuraActive("Nature's Swiftness", tar))
                    {
						...
						Blacklist.Add(tar, new TimeSpan(0, 0, 182));
                    }
					...
					else if (hp < HazzDruidSettings.Instance.WildGrowthPercent && !isAuraActive("Wild Growth", tar))
					{
					...
					else if (hp < HazzDruidSettings.Instance.HealingTouchPercent && HazzDruidSettings.Instance.UseNourish && !isAuraActive("Nourish", tar))
                    {
					...
					else if (hp < HazzDruidSettings.Instance.SwiftmendPercent && CC("Swiftmend") && isAuraActive("Rejuvenation", tar))
                    {
                        ...
                        s = "Swiftmend";
						Blacklist.Add(tar, new TimeSpan(0, 0, 16));
                    }
					if (s != null && CC(s, tar))
                    {
                        if (!C(s, tar))
        ...
        public override sealed string Name { get { return "HazzDruid EliT3.0"; } }
